...
- **Workflow:** gated task specs with STOP points → **CC works on a branch and hands back for review in chat.** Once the human has reviewed the work in chat, CC may commit to `main` (or fast-forward the branch into `main`) directly — a formal GitHub PR is **not** required unless the human asks for one. **The review gate is the in-chat handoff, not the PR.** Still stop at each acceptance gate before proceeding. **No AI authorship in commits** (no `Co-Authored-By:` trailers). Do not spend time on history tidying or rebases.
...
Current shape (verified 25 Jul 2026): 68 tables, 21 views, `raster_asset` 126 rows (18 at EPSG:8058, all extents populated), `figure_asset` 255, `report_asset` 59, `spatial_layer_asset` 6, `census_asset` 1.
