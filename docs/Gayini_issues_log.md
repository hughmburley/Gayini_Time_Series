...
| **I-27** | Docs | DEFECT | CLAUDE.md "Current state" understates two counts: `spatial_layer_asset` **6→9** (T12 Gate A0, `spatial_007/008/009`) and `raster_asset` **126→166** (already-stale 126; +38 T12 Gate A). Stale prose vs data — the exact discrepancy class the provenance rules target. | No | **Pending doc-sync. Do NOT edit CLAUDE.md piecemeal** — updated once at T12 close in a single edit. That edit also folds in (a) the Gate A `raster_asset` delta and (b) the standing-git-rule wording from I-28. Tracked here so the close-out edit is not forgotten |
