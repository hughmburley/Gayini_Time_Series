...
| **I-22** | D2 dashboards | DEFECT | D2 site dashboards are **57 of 66**, not "5 of 66" (CLAUDE.md stale). The 9 unbuilt are **exactly the `ground_cover_exclusion_flag = 1` plots** (GA_011/012/014/015/021/023/029/030/065 — treed Floodplain Woodland/Forest, the context/treed set the project sets aside). The shared composer errors `limits length 1` at `gayini_dashboard_panels.R:54` (date scale) because the excluded ground-cover panel collapses the aligned date range. | Maybe — only if the 9 excluded plots are wanted in the deck | **Design decision, not a mechanical fix** (touches the composer feeding the 57 deck dashboards): (a) leave unbuilt — they are the excluded set; (b) a ground-cover-free D2 variant; (c) length-guard the date scale so they render sans gc panel. Builder `scripts/07_figures_dashboards/13_build_missing_d2_dashboards.R` is ready once (b)/(c) is chosen |
