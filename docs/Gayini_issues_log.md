...
| **I-27** | Docs | DEFECT | CLAUDE.md "Current state" records `spatial_layer_asset` at **6 rows**; the live DB has **9** after T12 Gate A0 (added `spatial_007/008/009`). Stale prose vs data — the exact discrepancy class the provenance rules target. | No | **Pending doc-sync. Do NOT edit CLAUDE.md piecemeal** — per instruction it is updated once at T12 close in a single edit (which will also fold in the Gate A raster_asset delta). Tracked here so the close-out edit is not forgotten |
