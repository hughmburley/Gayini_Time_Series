...
This document records the active Gayini RS/hydrology workflow after final pruning of old numbered compatibility scripts. The top-level `scripts/` R workflow now ends at `10c`; historical scripts have been moved under `scripts/archive/pre_clean_spine_20260623/` for provenance and should not be run as active entry points.
...
7. `scripts/06_extract_MER_inundation_metrics.R`
...
8. `scripts/07_import_murrumbidgee_gauge_context.R`
...
13. `scripts/09b_check_curated_rs_hydrology_outputs.R`
    - Curated-output QA.
14. `scripts/10a_prepare_ground_cover_response_and_review_tables.R`
    - Prepares ground-cover response summaries and review tables.
15. `scripts/10b_make_review_figures_and_dashboards.R`
    - Builds review figures and dashboards, including gauge context figures where available.
16. `scripts/10c_make_review_extraction_method_maps.R`
...
The archive includes the former top-level `05b`, `06`, `06z`, `07`, `07z`, `08`, `09`, `10`, `11`, `13`, `15`, and `17a/17b/17c` scripts, plus retired dev/review utilities that were previously visible in the top-level `scripts/` folder.

Formal lag diagnostics are outside the active spine:

```text
scripts/downstream_stats/12_lag_diagnostics_inundation_gc.R
```
