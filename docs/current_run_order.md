...
5. `scripts/02_extract_heavy/02_extract_annual_inundation_full.R`
   - Annual Landsat inundation extraction.
6. `scripts/02_extract_heavy/03_extract_daily_inundation_full.R`
   - Daily inundation extraction.
7. `scripts/06_mer/01_extract_mer_inundation_metrics.R`
...
8. `scripts/04_gauges/01_import_murrumbidgee_gauge_context.R`
...
9. `scripts/03_inundation_products/01_build_prepost_inundation_products.R`
   - Builds pre/post annual inundation occurrence products.
   - Expensive raster-processing step; run only when explicitly needed.
10. `scripts/09_qa/01_check_prepost_inundation_products.R`
    - Read-only QA for existing pre/post inundation products.
    - Does not rebuild rasters.
11. `scripts/03_inundation_products/02_extract_prepost_inundation_to_plots.R`
    - Extracts fixed pre/post inundation products to plots.
12. `scripts/03_inundation_products/03_curate_rs_hydrology_analysis_base.R`
...
13. `scripts/09_qa/02_check_curated_rs_hydrology_outputs.R`
    - Curated-output QA.
14. `scripts/05_ground_cover/01_prepare_ground_cover_response_and_review_tables.R`
    - Prepares ground-cover response summaries and review tables.
15. `scripts/07_figures_dashboards/01_make_review_figures_and_dashboards.R`
    - Builds review figures and dashboards, including gauge context figures where available.
16. `scripts/07_figures_dashboards/02_make_review_extraction_method_maps.R`
    - Builds appendix/method map assets.

There is intentionally no parent pre/post wrapper script. The build, QA and plot-extraction steps are explicit entries in `docs/run_order/01_full_rebuild_workflow.csv` and `docs/run_order/04_qa_workflow.csv`.
