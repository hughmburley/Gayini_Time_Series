...
| **C6** | **Figure 19's two colour-scale labels are wrong, and so is its caption's flood clause — carried to V14. Ruling AY, 7 Aug 2026.** `M5_dual_grain_floor_and_flood.png` selects `AVG(veg_p05_spatial)` and labelled the cover scale **`veg_p05 (%)`** — the census *temporal* name on the *spatial* quantity, the pair CLAUDE.md calls the most confusable in the project. The flood scale is worse: it plots `AVG(flood_frac_pct)`, a share of **cells** wet within a year, under **`flood (% yrs)`**, which asserts a denominator of **years** and so names `census_flood_frequency_pct`, a per-cell property with no time axis. Both propagated into the registered caption, whose *"bottom row is how often the ground floods"* is the between-year framing of a within-year quantity. **Corrected at source in `build_T11_v2_dual_grain.R` and in the caption register.** The producer is NOT re-run, because it registers as it renders and registration is gated; **the methods document is sealed at V13 and is not edited.** The figure does not ship in pack v1.3, so nothing sent to Adrian carries either label. **V14 must take the corrected labels, the corrected caption, and a re-render.** | **OPEN — V14** |
| **C7** | **Figure 25's x-axis stated a denominator of years — CORRECTED AND RE-RENDERED. Ruling AZ, 7 Aug 2026.** `F5_cover_vs_water_64_paddocks.png` plotted `mean_flood` — `flood_frac_pct` averaged over 35 years, a share of the paddock's **cells** — under **"Mean annual flood frequency (% of years wet)"**, and its caption said *"largely set by how often it floods"*. Ruling AY's family, third instance, on the project's most-shown figure. **Its y-axis was already correct**, so one `labs()` call named the cover quantity precisely and got the water denominator wrong. Also corrected: three occurrences in `PACK1_build_workbook.py` — **which shipped in pack v1.2 and still carry the old wording there**. Unlike C6, this figure WAS re-rendered and its `figure_asset` row updated, authorised for this figure only because the uncorrected version is the one most likely to be presented on 10 August; checksum `72fa686516fb` → `5887de84db21`. **V13 still carries the uncorrected figure and stays sealed. V14 must take the re-render.** | **OPEN — V14** |
