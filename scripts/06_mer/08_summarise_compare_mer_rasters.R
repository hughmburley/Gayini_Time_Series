# ------------------------------------------------------------------------------
# Script: scripts/06_mer/08_summarise_compare_mer_rasters.R
# Purpose: Summarise and compare MER rasters.
# Workflow stage: 06_mer
# Run mode: lightweight_review
# Heavy processing: no
# Key inputs:
#   - Existing MER rasters and comparison tables.
# Key outputs:
#   - MER raster summary figures/tables.
# Notes:
...
if (length(missing_inputs) > 0) {
  stop("Missing required Task 12 inputs. Run scripts/06_mer/07_build_mer_annual_max_rasters.R first: ", paste(missing_inputs, collapse = "; "), call. = FALSE)
}
...
make_asset_row <- function(asset_id, path, title, priority, status, slide, notes) {
  ...
  tibble::tibble(
    ...
    source_script = "scripts/06_mer/07_build_mer_annual_max_rasters.R; scripts/06_mer/08_summarise_compare_mer_rasters.R",
