...
## 25_build_mer_annual_max_rasters.R
## -----------------------------------------------------------------------------
...
if (length(missing_inputs) > 0) {
  stop("Missing Task 10 census outputs. Run scripts/06_mer/24a_census_mer_raster_inputs.R first: ", paste(missing_inputs, collapse = "; "), call. = FALSE)
}
...
output_manifest <- dplyr::bind_rows(annual_output_manifest, period_output_manifest) %>%
  dplyr::mutate(
    created_by = "scripts/06_mer/25_build_mer_annual_max_rasters.R",
