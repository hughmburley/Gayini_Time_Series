...
The scripts folder is grouped by workflow role and active scripts are numbered
sequentially inside each folder. Start from `run_spine_smoke_test.R`, then use
`docs/run_order/README.md` and `docs/current_run_order.md` for the canonical run
order. Use `docs/scripts_manifest.csv` and `docs/script_rename_map_stage4.csv`
for classification and rename records.
...
- `02_extract_heavy/`: heavy ground-cover, annual inundation and daily inundation extraction. The `internal/` folder holds tracked implementation scripts used by the active wrappers.
...
figures. The default spine smoke test parses and checks scripts but does not run
the workflow. Local/off-repo packaging utilities were removed from the shared
branch in Stage 3 and documented in `docs/scripts_manifest.csv`.
