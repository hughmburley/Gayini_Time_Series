...
RUN_ID  <- "T1_gateB1"
SUPPORT <- "zone"            # management-zone unit (not pixel/plot); stated in every caption
LEVEL   <- "diagnostics"
...
p1 <- ggplot() +
  ...
  labs(title = "T1 A0 · 8058 zone layer over the census grid extent",
       ...
       caption = "Support: zone (management-zone unit). Both layers EPSG:8058.") +
  theme_minimal(base_size = 11)
gayini_write_and_register_figure(
  p1, file.path(fig_dir, "T1_A0_zone_layer_extent.png"),
  title = "T1 A0 zone layer extent",
  caption = "Support: zone. 8058 zone layer over the census grid extent; extents coincide.",
...
p2 <- ggplot(z) +
  ...
  labs(title = "T1 A · Management zones, labelled with paddock names",
       ...
       caption = "Support: zone. Labels are ManagmentZ paddock names, not indices.") +
  theme_minimal(base_size = 11)
gayini_write_and_register_figure(
  p2, file.path(fig_dir, "T1_A_zone_map_named.png"),
  title = "T1 A zone map named",
  caption = "Support: zone. All 64 zones filled by treatment, labelled with paddock names.",
  support_level = SUPPORT, figure_level = LEVEL, run_id = RUN_ID,
...
p3 <- ggplot(long, aes(fid, value)) +
  ...
  labs(title = "T1 A · Zone identity — residual band and per-zone margins",
       ...
       x = "zone fid", y = NULL,
       caption = "Support: zone. Residual = |MODIS area − computed area| / MODIS area. Margin < 0 = a competitor fid is closer (area-twin).") +
  theme_minimal(base_size = 11) + theme(legend.position = "top")
gayini_write_and_register_figure(
  p3, file.path(fig_dir, "T1_A_identity_margin.png"),
  title = "T1 A identity margin",
  caption = "Support: zone. Assumed-partner residual band and per-zone margins; fid 9/21 marked as area-twins.",
  support_level = SUPPORT, figure_level = LEVEL, run_id = RUN_ID,
...
p4 <- ggplot(areas, aes(area_ha_source, area_ha_computed)) +
  ...
  labs(title = "T1 B · Area_MW (source) vs area_ha_computed (8058 geometry)",
       ...
       caption = "Support: zone. Both area columns retained in dim_management_zone with area_ha_diff_pct.") +
...
gayini_write_and_register_figure(
  ...
  caption = "Support: zone. Area_MW vs area_ha_computed, 1:1 line; systematic projection offset annotated.",
  support_level = SUPPORT, figure_level = LEVEL, run_id = RUN_ID,
