...
$resolvedPackageRoot = if (Test-Path -LiteralPath $PackageRoot) {
...
$packageName = "Gayini_MER_RS_latest_run_outputs_$timestamp"
...
$relativeTargets = @(
    ...
    "Output\diagnostics\hydrology\hydrology_import_checks.csv",
    "Output\diagnostics\hydrology\hydrology_imported_source_files.csv",
    "Output\diagnostics\hydrology\hydrology_gauge_role_lookup.csv",
    "Output\diagnostics\hydrology\hydrology_join_key_checks.csv",
    "Output\diagnostics\hydrology\rs_gauge_join_summary.csv",
...
if ((Split-Path -Leaf $resolvedStagingRoot) -notlike "_staging_Gayini_MER_RS_latest_run_outputs_*") {
