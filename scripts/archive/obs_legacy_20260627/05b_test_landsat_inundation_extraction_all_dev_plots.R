...
## 05b_test_landsat_inundation_extraction_all_dev_plots.R
## -----------------------------------------------------------------------------


## Purpose:
## Scale the Landsat annual inundation extraction from the 05a 10-plot smoke
## test to all clean hectare plots, while still using only the development raster
## subset.
...
## This script is still a test step. It does not run the full annual inundation
## archive yet. It checks whether the count-area extraction behaves sensibly
## across all 66 plots before we scale to 05c.
...
if (!exists("gayini_extract_landsat_inundation_collection", mode = "function")) {
  stop(
    "Required function gayini_extract_landsat_inundation_collection() was not loaded. Check R/inundation_extraction_functions.R.",
    call. = FALSE
  )
}
...
## Use all clean hectare plots, but still only the development raster subset.
## This scales one dimension at a time, following the ground-cover workflow.
...
output_path <- file.path(
  output_csv_dir,
  "05b_landsat_inundation_all_dev_plots.csv"
)

checks_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_checks.csv"
)

count_summary_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_count_summary.csv"
)

coverage_summary_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_coverage_summary.csv"
)

plot_summary_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_plot_summary.csv"
)

treatment_summary_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_treatment_summary.csv"
)

coverage_status_summary_path <- file.path(
  output_diagnostics_dir,
  "05b_landsat_inundation_all_dev_plots_coverage_status_summary.csv"
)

inundated_heatmap_path <- file.path(
  output_figures_dir,
  "05b_landsat_inundation_all_dev_plots_inundated_any_heatmap.png"
)

treatment_timeseries_path <- file.path(
  output_figures_dir,
  "05b_landsat_inundation_all_dev_plots_mean_inundated_any_by_treatment.png"
)

coverage_heatmap_path <- file.path(
  output_figures_dir,
  "05b_landsat_inundation_all_dev_plots_valid_coverage_heatmap.png"
)

count_area_treatment_path <- file.path(
  output_figures_dir,
  "05b_landsat_inundation_all_dev_plots_count_area_by_treatment.png"
)

majority_count_heatmap_path <- file.path(
  output_figures_dir,
  "05b_landsat_inundation_all_dev_plots_majority_count_heatmap.png"
)
...
if (nrow(landsat_inundation_catalog) == 0) {
  stop(
    "No Landsat annual inundation rasters found in the development raster subset.",
    call. = FALSE
  )
}
...
message("Landsat annual inundation development rasters: ", nrow(landsat_inundation_catalog))
message(
...
extraction_output <- gayini_extract_landsat_inundation_collection(
  ...
  extraction_scope = "landsat_inundation_all_dev_plots",
...
if (length(missing_count_output_columns) > 0) {
  stop(
    "The 05b extraction output is missing required count-based columns: ",
...
if (length(legacy_class_columns) > 0) {
  warning(
    "Legacy class-based columns are present in 05b output: ",
...
inundated_heatmap <- ggplot2::ggplot(
  ...
  ggplot2::geom_tile() +
  ggplot2::labs(
    title = "05b Landsat annual inundation all-dev test: inundated area",
    subtitle = "All plots; development annual rasters only; inundated_any_pct = area where annual count > 0",
    x = "Raster start date",
    y = "Plot ID",
    fill = "Inundated area (%)"
  ) +
...
treatment_timeseries <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "05b Landsat annual inundation all-dev test: mean inundated area by treatment",
...
coverage_heatmap <- ggplot2::ggplot(
  ...
  ggplot2::aes(
    x = .data$date_start,
    y = .data$plot_id,
    fill = .data$valid_coverage_pct
  )
) +
  ggplot2::geom_tile() +
  ggplot2::geom_hline(yintercept = 0, alpha = 0) +
  ggplot2::labs(
    title = "05b Landsat annual inundation all-dev test: valid coverage",
...
count_area_treatment_plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "05b Landsat annual inundation all-dev test: mean count-area percentages by treatment",
...
majority_count_heatmap <- ggplot2::ggplot(
  ...
  ggplot2::geom_tile() +
  ggplot2::labs(
    title = "05b Landsat annual inundation all-dev test: majority count",
...
message("Landsat inundation 05b all-dev-plot test complete.")
message("Rows written: ", nrow(extraction_output))
message("Expected rows: ", expected_rows)
message("Review CSV outputs and diagnostic plots before running full Landsat inundation extraction.")

if (has_warning) {
  warning(
    "Landsat inundation 05b warnings were created. Review: ",
    checks_path,
    call. = FALSE
  )
}
