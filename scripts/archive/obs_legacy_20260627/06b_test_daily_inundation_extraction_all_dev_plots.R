## 06b daily inundation extraction all-dev-plot test ----

## Purpose:
## Scale the passed 06a daily inundation extraction from 10 plots to all clean plots.
## This still uses only the development daily-inundation raster subset, not the full
## daily time series.
...
raster_dev_subset_path   <- file.path(root_dir, "data_intermediate", "raster_catalog", "raster_dev_subset.csv")
...
main_output_path         <- file.path(csv_dir, "06b_daily_inundation_all_dev_plots.csv")

processed_output_path    <- file.path(root_dir, "data_processed", "daily_inundation_dev_plot_timeseries.csv")

selected_rasters_path    <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_selected_rasters.csv")

checks_path              <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_checks.csv")

sensor_summary_path      <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_sensor_summary.csv")

value_summary_path       <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_value_summary.csv")

coverage_summary_path    <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_coverage_summary.csv")

plot_summary_path        <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_plot_summary.csv")

sensor_date_summary_path <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_sensor_date_summary.csv")

value_by_sensor_path     <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_value_by_sensor_summary.csv")

top_events_path          <- file.path(diagnostics_dir, "06b_daily_inundation_all_dev_plots_top_inundation_events.csv")


## Figure paths ----

fig_inundated_heatmap_path <- file.path(figures_dir, "06b_daily_inundation_all_dev_plots_daily_inundated_heatmap.png")

fig_sensor_summary_path    <- file.path(figures_dir, "06b_daily_inundation_all_dev_plots_mean_inundated_by_sensor.png")

fig_value_by_sensor_path   <- file.path(figures_dir, "06b_daily_inundation_all_dev_plots_value_area_by_sensor.png")

fig_coverage_heatmap_path  <- file.path(figures_dir, "06b_daily_inundation_all_dev_plots_valid_interpretation_heatmap.png")

fig_cloud_heatmap_path     <- file.path(figures_dir, "06b_daily_inundation_all_dev_plots_cloud_shadow_heatmap.png")
...
if (!file.exists(raster_dev_subset_path)) {
  stop("Missing raster development subset: ", raster_dev_subset_path, call. = FALSE)
}

plots_clean <- sf::st_read(plots_clean_path, quiet = TRUE)

raster_dev_subset <- readr::read_csv(raster_dev_subset_path, show_col_types = FALSE)
...
## Standardise daily inundation development raster subset ----

if (!"plot_id" %in% names(plots_clean)) {
  stop("Clean plots must contain a plot_id column.", call. = FALSE)
}

daily_inundation_dev_catalog <- gayini_standardise_daily_inundation_catalog(
  raster_catalog = raster_dev_subset,
  root           = root_dir
)

daily_inundation_dev_catalog <- daily_inundation_dev_catalog |>
  dplyr::distinct(.data$file_path, .keep_all = TRUE) |>
  dplyr::arrange(.data$date_start, .data$sensor, .data$file_name)

message("Daily inundation development rasters: ", nrow(daily_inundation_dev_catalog))

message("Daily inundation development sensors: ", paste(unique(daily_inundation_dev_catalog$sensor), collapse = ", "))

if (nrow(daily_inundation_dev_catalog) == 0) {
  stop("No daily inundation rasters found in raster_dev_subset.csv.", call. = FALSE)
}
...
selected_rasters <- daily_inundation_dev_catalog

message("Selected all plots: ", nrow(selected_plots))

message("Selected daily inundation development rasters: ", nrow(selected_rasters))
...
fig_inundated_heatmap <- ggplot(extraction, aes(x = date_start, y = plot_id, fill = daily_inundated_pct)) +
  ...
  labs(
    title = "06b daily inundation all-dev-plot test: area inundated",
    subtitle = "Primary metric: daily_inundated_pct = area where value == 1; all plots, development daily rasters only",
    x = "Raster date",
    y = "Plot",
    fill = "Inundated (%)"
  ) +
...
ggsave(fig_inundated_heatmap_path, fig_inundated_heatmap, width = 13, height = 14, dpi = 300)
...
fig_sensor_summary <- ggplot(sensor_date_summary, aes(x = date_start, y = mean_daily_inundated_pct, group = sensor)) +
  ...
  labs(
    title = "06b daily inundation all-dev-plot test: mean inundation by sensor",
...
ggsave(fig_sensor_summary_path, fig_sensor_summary, width = 12, height = 7, dpi = 300)
...
fig_value_by_sensor <- ggplot(value_date_sensor_summary, aes(x = date_start, y = mean_area_pct, group = sensor)) +
  ...
  labs(
    title = "06b daily inundation all-dev-plot test: mean value-area percentages",
...
ggsave(fig_value_by_sensor_path, fig_value_by_sensor, width = 13, height = 8, dpi = 300)
...
fig_coverage_heatmap <- ggplot(extraction, aes(x = date_start, y = plot_id, fill = valid_interpretation_pct)) +
  ...
  facet_grid(sensor ~ ., scales = "free_y", space = "free_y") +
  labs(
    title = "06b daily inundation all-dev-plot test: valid interpretation area",
...
ggsave(fig_coverage_heatmap_path, fig_coverage_heatmap, width = 13, height = 14, dpi = 300)
...
fig_cloud_heatmap <- ggplot(extraction, aes(x = date_start, y = plot_id, fill = value_3_cloud_shadow_pct)) +
  ...
  facet_grid(sensor ~ ., scales = "free_y", space = "free_y") +
  labs(
    title = "06b daily inundation all-dev-plot test: cloud shadow area",
...
ggsave(fig_cloud_heatmap_path, fig_cloud_heatmap, width = 13, height = 14, dpi = 300)
...
message("Daily inundation 06b all-dev-plot test complete.")
...
message("Review diagnostics and figures before scaling to 06c.")

if (any(checks$check == "row_count_matches" & checks$value == "FALSE")) {
  warning("Daily inundation 06b row count did not match expectation. Review checks before continuing.", call. = FALSE)
}
