...
generating_script_guess <- function(path) {
  nm <- basename(path)
  dplyr::case_when(
    stringr::str_detect(nm, "plot_treed") ~ "scripts/10d_prepare_plot_context_flags.R",
    stringr::str_detect(nm, "gauge|dashboard_gauge|with_gauge|selected_plot_total_veg") ~ "scripts/10e_integrate_gauge_context_review_figures.R",
    stringr::str_detect(nm, "background|matched|inundation_frequency_by_vegetation") ~ "scripts/10f_prepare_background_flood_pattern_matched_years.R",
    stringr::str_detect(nm, "mer_metric_summary") ~ "scripts/10g_consolidate_mer_metric_review.R",
    stringr::str_detect(nm, "MER|mer") ~ "scripts/06_extract_MER_inundation_metrics.R",
    stringr::str_detect(nm, "10b_") ~ "scripts/10b_make_review_figures_and_dashboards.R",
    stringr::str_detect(nm, "modis") ~ "scripts/03_extract_ground_cover_full.R",
    stringr::str_detect(nm, "05c|06c|04c") ~ "scripts/03_extract_ground_cover_full.R; scripts/04_extract_annual_inundation_full.R; scripts/05_extract_daily_inundation_full.R",
