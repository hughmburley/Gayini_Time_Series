...
## These helpers make the fractional-cover extraction robust to small changes in
## the raster catalogue schema. The full raster catalogue should ideally include
## product and sensor columns, but older catalogues may not.
...
gayini_infer_fractional_cover_product <- function(file_path, file_name) {
  file_path_lower <- tolower(file_path)
  file_name_lower <- tolower(file_name)

  dplyr::case_when(
    grepl("landsat_fractionalcover3", file_path_lower) ~ "landsat_fractional_cover",
    grepl("landsat[_-]?fractional", file_path_lower) ~ "landsat_fractional_cover",
    grepl("fractional[_-]?cover", file_path_lower) ~ "landsat_fractional_cover",
    grepl("^lztmre_", file_name_lower) ~ "landsat_fractional_cover",
    TRUE ~ NA_character_
  )
}


gayini_infer_fractional_cover_sensor <- function(file_path, file_name) {
  file_path_lower <- tolower(file_path)
  file_name_lower <- tolower(file_name)
  combined_text    <- paste(file_path_lower, file_name_lower)

  dplyr::case_when(
    grepl("landsat[_ -]?9|\\bl9\\b|_l9_", combined_text) ~ "L9",
    grepl("landsat[_ -]?8|\\bl8\\b|_l8_", combined_text) ~ "L8",
    grepl("landsat[_ -]?7|\\bl7\\b|_l7_", combined_text) ~ "L7",
    grepl("landsat[_ -]?5|\\bl5\\b|_l5_", combined_text) ~ "L5",
    grepl("landsat|lztmre|fractionalcover", combined_text) ~ "LS",
    TRUE ~ NA_character_
  )
}


gayini_standardise_fractional_cover_catalog <- function(raster_catalog,
  ...
  product_source_column <- gayini_first_existing_column(
    output_catalog,
    c("product", "raster_product", "product_name", "product_type", "dev_subset_group")
  )

  inferred_product <- gayini_infer_fractional_cover_product(
  ...
  if (is.na(product_source_column)) {
  ...
  } else if (product_source_column != "product") {
    ...
    output_catalog$product <- as.character(output_catalog$product)
    output_catalog$product <- dplyr::if_else(
      is.na(output_catalog$product) | output_catalog$product == "",
      inferred_product,
      output_catalog$product
  ...
  sensor_source_column <- gayini_first_existing_column(
    output_catalog,
    c("sensor", "satellite", "platform", "sensor_platform")
  )

  inferred_sensor <- gayini_infer_fractional_cover_sensor(
    file_path = output_catalog$file_path,
    file_name = output_catalog$file_name
  )

  if (is.na(sensor_source_column)) {
    output_catalog$sensor <- inferred_sensor
  } else if (sensor_source_column != "sensor") {
    ...
    output_catalog$sensor <- as.character(output_catalog$sensor)
    output_catalog$sensor <- dplyr::if_else(
      is.na(output_catalog$sensor) | output_catalog$sensor == "",
      inferred_sensor,
      output_catalog$sensor
  ...
  output_catalog <- output_catalog |>
    dplyr::mutate(
      product = dplyr::if_else(
        is.na(.data$product) & !is.na(inferred_product),
        inferred_product,
        .data$product
      ),
      sensor = dplyr::if_else(
        .data$product == target_product & (is.na(.data$sensor) | .data$sensor == ""),
        "LS",
        .data$sensor
      )
    )
...
gayini_extract_fractional_cover_raster <- function(raster_row,
  ...
  raster_row <- gayini_standardise_fractional_cover_catalog(raster_row)
...
gayini_extract_fractional_cover_subset <- function(raster_subset,
                                                   ...
                                                   value_scale_factor = 1,
                                                   extraction_scope = "development_smoke_test") {
  raster_subset <- gayini_standardise_fractional_cover_catalog(raster_subset)
