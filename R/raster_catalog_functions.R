...
gayini_parse_modis_version <- function(path_or_name) {

  filename <- basename(path_or_name)
  match <- stringr::str_match(tolower(filename), "aust\\.([0-9]{3})\\.(tif|tiff)$")

  ifelse(is.na(match[, 1]), NA_character_, match[, 2])
...
gayini_standardise_daily_inundation_sensor <- function(sensor_token) {

  if (is.na(sensor_token) || sensor_token == "") {
    return("unknown")
  }

  sensor_token <- tolower(sensor_token)

  if (sensor_token %in% c("s2", "sentinel2", "sentinel_2")) {
    return("s2")
  }

  if (sensor_token %in% c("l8", "landsat8", "landsat_8")) {
    return("l8")
  }

  if (sensor_token %in% c("l7", "landsat7", "landsat_7")) {
    return("l7")
  }

  if (sensor_token %in% c("inundation", "water")) {
    return("unknown")
  }
...
gayini_parse_raster_dates <- function(path) {
  ...
  if (product == "sentinel2_inundation") {
    ...
    ## This folder is named sentinel2_inundation, but it contains daily inundation files
    ## from multiple sensors/patterns, including S2, L8, L7 and generic inundation files.

    match <- stringr::str_match(filename_lower, "lo_([0-9]{8})(?:_([a-z0-9]+))?")

    if (!is.na(match[1, 1])) {
      date_start  <- gayini_safe_ymd(match[1, 2])
      date_end    <- date_start
      water_year  <- gayini_make_water_year(date_start)
      sensor      <- gayini_standardise_daily_inundation_sensor(match[1, 3])

      parse_status <- ifelse(
        is.na(date_start),
        "date_invalid_daily_inundation",
        paste0("parsed_daily_inundation_", sensor)
      )
    }
...
gayini_catalog_one_raster <- function(path) {
  ...
  dplyr::bind_cols(result, dplyr::select(date_info, -product, -sensor))
...
gayini_catalog_rasters <- function(root = getwd()) {
  ...
  catalog <- catalog |>
    dplyr::mutate(
      modis_version          = dplyr::if_else(.data$product == "modis_fractional_cover", gayini_parse_modis_version(.data$file_name), NA_character_),
...
gayini_make_modis_catalog_checks <- function(catalog) {

  modis_catalog <- catalog |>
    dplyr::filter(.data$product == "modis_fractional_cover") |>
    dplyr::mutate(
      date_start = as.Date(.data$date_start),
      year_month = ifelse(is.na(.data$date_start), NA_character_, format(.data$date_start, "%Y-%m")),
      modis_version = dplyr::if_else(
        is.na(.data$modis_version) | .data$modis_version == "",
        gayini_parse_modis_version(.data$file_name),
        .data$modis_version
      )
    ) |>
  ...
  if (nrow(modis_catalog) == 0) {
    return(tibble::tibble(
      ...
      modis_version = NA_character_,
      n_layers = NA_integer_,
      nodata_value = NA_character_,
      crs_name = NA_character_,
      read_status = NA_character_,
      duplicate_year_month = NA,
      status = "fail",
      issues = "no_modis_fractional_cover_files_detected",
  ...
  modis_catalog |>
    dplyr::mutate(
      has_three_layers = !is.na(.data$n_layers) & .data$n_layers == 3,
      has_nodata_255 = !is.na(.data$nodata_value) & grepl("255", as.character(.data$nodata_value)),
      has_crs = (!is.na(.data$crs_name) & .data$crs_name != "") | (!is.na(.data$crs_epsg) & .data$crs_epsg != ""),
      has_date_start = !is.na(.data$date_start),
      has_modis_version = !is.na(.data$modis_version) & .data$modis_version != "",
      issues = purrr::pmap_chr(
        list(.data$read_status, .data$has_three_layers, .data$has_nodata_255, .data$has_crs, .data$has_date_start, .data$has_modis_version, .data$duplicate_year_month),
        function(read_status, has_three_layers, has_nodata_255, has_crs, has_date_start, has_modis_version, duplicate_year_month) {
          issue_values <- character(0)

          if (!identical(read_status, "ok")) issue_values <- c(issue_values, "read_failed")
          if (!isTRUE(has_three_layers)) issue_values <- c(issue_values, "n_layers_not_3")
          if (!isTRUE(has_nodata_255)) issue_values <- c(issue_values, "nodata_255_not_declared")
          if (!isTRUE(has_crs)) issue_values <- c(issue_values, "crs_missing")
          if (!isTRUE(has_date_start)) issue_values <- c(issue_values, "date_start_not_parsed")
          if (!isTRUE(has_modis_version)) issue_values <- c(issue_values, "modis_version_not_parsed")
          if (isTRUE(duplicate_year_month)) issue_values <- c(issue_values, "duplicate_year_month")

          if (length(issue_values) == 0) {
            return("none")
          }

          paste(issue_values, collapse = "; ")
        }
      ),
      status = dplyr::case_when(
        grepl("read_failed|n_layers_not_3|crs_missing|date_start_not_parsed", .data$issues) ~ "fail",
        .data$issues != "none" ~ "warn",
        TRUE ~ "pass"
      )
    ) |>
    dplyr::select(
    ...
    dplyr::arrange(.data$date_start, .data$modis_version, .data$file_name)

}


gayini_make_modis_catalog_summary <- function(modis_checks) {

  if (nrow(modis_checks) == 1 && identical(modis_checks$status[1], "fail") && identical(modis_checks$issues[1], "no_modis_fractional_cover_files_detected")) {
    return(tibble::tibble(
      summary_group = "all_modis",
      file_count = 0,
      first_date = as.Date(NA),
      last_date = as.Date(NA),
      modis_versions = NA_character_,
      n_failed_checks = 1,
      n_warning_checks = 0,
      n_duplicate_year_months = 0
    ))
  }
  ...
  modis_checks |>
    dplyr::summarise(
      summary_group = "all_modis",
      file_count = dplyr::n(),
      first_date = min(.data$date_start, na.rm = TRUE),
      last_date = max(.data$date_start, na.rm = TRUE),
      modis_versions = paste(sort(unique(stats::na.omit(.data$modis_version))), collapse = "; "),
      n_failed_checks = sum(.data$status == "fail", na.rm = TRUE),
      n_warning_checks = sum(.data$status == "warn", na.rm = TRUE),
      n_duplicate_year_months = sum(.data$duplicate_year_month, na.rm = TRUE)
    )
...
gayini_make_raster_catalog_summaries <- function(catalog) {
  ...
  sensor_summary <- catalog |>
    dplyr::group_by(product, sensor) |>
    dplyr::summarise(
      file_count          = dplyr::n(),
      total_size_gb       = round(sum(file_size_mb, na.rm = TRUE) / 1024, 3),
      date_parse_failures = sum(needs_date_check, na.rm = TRUE),
    ...
    dplyr::arrange(product, sensor)

  warnings <- catalog |>
    dplyr::filter(read_status != "ok" | needs_date_check | is.na(crs_name) | product == "unknown") |>
    dplyr::select(file_name, product, sensor, read_status, date_parse_status, crs_name, file_path)

  modis_checks <- gayini_make_modis_catalog_checks(catalog)
  modis_summary <- gayini_make_modis_catalog_summary(modis_checks)

  list(
    product_summary = product_summary,
    sensor_summary  = sensor_summary,
    warnings        = warnings,
    modis_checks    = modis_checks,
...
gayini_write_raster_catalog_outputs <- function(catalog, root = getwd()) {
  ...
  gayini_write_csv(summaries$sensor_summary, file.path(csv_dir, "raster_sensor_summary.csv"))
  gayini_write_csv(summaries$warnings, file.path(diagnostics_dir, "raster_catalog_warnings.csv"))
  gayini_write_csv(summaries$modis_checks, file.path(diagnostics_dir, "modis_fractional_cover_catalog_checks.csv"))
  gayini_write_csv(summaries$modis_summary, file.path(csv_dir, "modis_fractional_cover_summary.csv"))
